...
FObj::FObj(float x, float y, float acc, int typ) {
...
void FObj::set_position(float x, float y) {
    this->position = glm::vec3(x, y, 0);
}
...
void FObj::tick() {
    ...
    if(this->position.y > center + 0.3) this->speed_y = -this->speed_y; 
    else if(this->position.y < center - 0.3) this->speed_y = -this->speed_y; 
}

bounding_box_t FObj::bbox() {
    bounding_box_t a;
    a.x = this->position.x;
    a.y = this->position.y;
    a.height = 0.4;
