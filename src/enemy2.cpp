...
Enemy::Enemy(float x, float y, color_t color) {
    ...
    this->cir1 = create3DObject(GL_TRIANGLES, n*3, vertex_buffer_data1, COLOR_YELLOW, GL_FILL);
    ...
    this->cir2 = create3DObject(GL_TRIANGLES, n*3, vertex_buffer_data1, COLOR_YELLOW, GL_FILL);
}
...
void Enemy::tick(){
    ...
    if(abs(this->position.y - mid) >= 1.0) speed_y = -speed_y;
}
